      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <div className="mb-3 flex items-center justify-between lg:hidden">
            ...
            <div className="text-[11px] uppercase tracking-[0.35em] text-muted-foreground">GPMS</div>
          </div>
          <div className="text-[11px] uppercase tracking-[0.35em] text-muted-foreground">GPMS Control Panel</div>
          <motion.h1 key={pathname} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-1 text-2xl font-semibold tracking-tight">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:flex xl:items-center">
          ...
          <label className="flex h-10 min-w-[180px] items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 text-sm text-muted-foreground">
            ...
            <input className="w-full bg-transparent text-foreground outline-none" type="date" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} />
          </label>
          ...
          <Button variant="ghost" size="sm" onClick={toggleTheme}>{theme === "dark" ? <SunMedium className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</Button>
          <Button variant="ghost" size="sm"><Bell className="h-4 w-4" /></Button>
          <button className="flex h-10 items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-3 text-left" onClick={() => router.push("/dashboard")} type="button">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary/20 text-xs font-semibold">{user?.name?.slice(0, 1).toUpperCase()}</div>
